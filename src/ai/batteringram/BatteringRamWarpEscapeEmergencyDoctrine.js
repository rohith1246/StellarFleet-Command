/**
 * BatteringRamWarpEscapeEmergencyDoctrine.js - Fleet Command Tactical AI Doctrine: BatteringRam -> WarpEscapeEmergency.
 */

class BatteringRamWarpEscapeEmergencyDoctrine {
  constructor(fleetCommander, sensorRange = 50000) {
    this.doctrineName = 'BatteringRam';
    this.tacticName = 'WarpEscapeEmergency';
    this.commander = fleetCommander;
    this.sensorRange = sensorRange;
    this.aggressionFactor = 0.50;
  }
  ...
  evaluateBattlefield(hostileFleets, friendlyFleet) {
    ...
    return { action: 'CLOSE_DISTANCE_FORMATION', formation: 'BatteringRam', target: targetFleet };
  }
  ...
  execute(friendlyFleet, hostileFleets) {
    ...
    if (decision.action === 'ENGAGE_TACTICAL_MANOEUVRE') {
      ...
      friendlyFleet.repositionTowards(decision.target.pos, 'BatteringRam');
    }
    return decision;
  }
}

module.exports = { BatteringRamWarpEscapeEmergencyDoctrine };
