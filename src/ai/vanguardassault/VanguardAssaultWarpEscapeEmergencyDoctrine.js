/**
 * VanguardAssaultWarpEscapeEmergencyDoctrine.js - Fleet Command Tactical AI Doctrine: VanguardAssault -> WarpEscapeEmergency.
 */

class VanguardAssaultWarpEscapeEmergencyDoctrine {
  constructor(fleetCommander, sensorRange = 50000) {
    this.doctrineName = 'VanguardAssault';
    this.tacticName = 'WarpEscapeEmergency';
    this.commander = fleetCommander;
    this.sensorRange = sensorRange;
    this.aggressionFactor = 0.30;
  }
  ...
  evaluateBattlefield(hostileFleets, friendlyFleet) {
    ...
    return { action: 'CLOSE_DISTANCE_FORMATION', formation: 'VanguardAssault', target: targetFleet };
  }
  ...
  execute(friendlyFleet, hostileFleets) {
    ...
    if (decision.action === 'ENGAGE_TACTICAL_MANOEUVRE') {
      ...
      friendlyFleet.repositionTowards(decision.target.pos, 'VanguardAssault');
    }
    return decision;
  }
}

module.exports = { VanguardAssaultWarpEscapeEmergencyDoctrine };
