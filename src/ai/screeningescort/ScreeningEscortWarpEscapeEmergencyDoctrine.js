/**
 * ScreeningEscortWarpEscapeEmergencyDoctrine.js - Fleet Command Tactical AI Doctrine: ScreeningEscort -> WarpEscapeEmergency.
 */

class ScreeningEscortWarpEscapeEmergencyDoctrine {
  constructor(fleetCommander, sensorRange = 50000) {
    this.doctrineName = 'ScreeningEscort';
    this.tacticName = 'WarpEscapeEmergency';
    this.commander = fleetCommander;
    this.sensorRange = sensorRange;
    this.aggressionFactor = 0.70;
  }
  ...
  evaluateBattlefield(hostileFleets, friendlyFleet) {
    ...
    return { action: 'CLOSE_DISTANCE_FORMATION', formation: 'ScreeningEscort', target: targetFleet };
  }
  ...
  execute(friendlyFleet, hostileFleets) {
    ...
    if (decision.action === 'ENGAGE_TACTICAL_MANOEUVRE') {
      ...
      friendlyFleet.repositionTowards(decision.target.pos, 'ScreeningEscort');
    }
    return decision;
  }
}

module.exports = { ScreeningEscortWarpEscapeEmergencyDoctrine };
