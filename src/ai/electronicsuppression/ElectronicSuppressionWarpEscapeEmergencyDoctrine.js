/**
 * ElectronicSuppressionWarpEscapeEmergencyDoctrine.js - Fleet Command Tactical AI Doctrine: ElectronicSuppression -> WarpEscapeEmergency.
 */

class ElectronicSuppressionWarpEscapeEmergencyDoctrine {
  constructor(fleetCommander, sensorRange = 50000) {
    this.doctrineName = 'ElectronicSuppression';
    this.tacticName = 'WarpEscapeEmergency';
    this.commander = fleetCommander;
    this.sensorRange = sensorRange;
    this.aggressionFactor = 0.58;
  }
  ...
  evaluateBattlefield(hostileFleets, friendlyFleet) {
    ...
    return { action: 'CLOSE_DISTANCE_FORMATION', formation: 'ElectronicSuppression', target: targetFleet };
  }
  ...
  execute(friendlyFleet, hostileFleets) {
    ...
    if (decision.action === 'ENGAGE_TACTICAL_MANOEUVRE') {
      ...
      friendlyFleet.repositionTowards(decision.target.pos, 'ElectronicSuppression');
    }
    return decision;
  }
}

module.exports = { ElectronicSuppressionWarpEscapeEmergencyDoctrine };
