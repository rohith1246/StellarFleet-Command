/**
 * MinefieldTetherWarpEscapeEmergencyDoctrine.js - Fleet Command Tactical AI Doctrine: MinefieldTether -> WarpEscapeEmergency.
 */

class MinefieldTetherWarpEscapeEmergencyDoctrine {
  constructor(fleetCommander, sensorRange = 50000) {
    this.doctrineName = 'MinefieldTether';
    this.tacticName = 'WarpEscapeEmergency';
    this.commander = fleetCommander;
    this.sensorRange = sensorRange;
    this.aggressionFactor = 0.78;
  }
  ...
  evaluateBattlefield(hostileFleets, friendlyFleet) {
    ...
    return { action: 'CLOSE_DISTANCE_FORMATION', formation: 'MinefieldTether', target: targetFleet };
  }
  ...
  execute(friendlyFleet, hostileFleets) {
    ...
    if (decision.action === 'ENGAGE_TACTICAL_MANOEUVRE') {
      ...
      friendlyFleet.repositionTowards(decision.target.pos, 'MinefieldTether');
    }
    return decision;
  }
}

module.exports = { MinefieldTetherWarpEscapeEmergencyDoctrine };
