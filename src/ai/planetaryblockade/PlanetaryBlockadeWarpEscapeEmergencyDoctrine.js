/**
 * PlanetaryBlockadeWarpEscapeEmergencyDoctrine.js - Fleet Command Tactical AI Doctrine: PlanetaryBlockade -> WarpEscapeEmergency.
 */

class PlanetaryBlockadeWarpEscapeEmergencyDoctrine {
  constructor(fleetCommander, sensorRange = 50000) {
    this.doctrineName = 'PlanetaryBlockade';
    this.tacticName = 'WarpEscapeEmergency';
    this.commander = fleetCommander;
    this.sensorRange = sensorRange;
    this.aggressionFactor = 0.90;
  }
  ...
  evaluateBattlefield(hostileFleets, friendlyFleet) {
    ...
    return { action: 'CLOSE_DISTANCE_FORMATION', formation: 'PlanetaryBlockade', target: targetFleet };
  }
  ...
  execute(friendlyFleet, hostileFleets) {
    ...
    if (decision.action === 'ENGAGE_TACTICAL_MANOEUVRE') {
      ...
      friendlyFleet.repositionTowards(decision.target.pos, 'PlanetaryBlockade');
    }
    return decision;
  }
}

module.exports = { PlanetaryBlockadeWarpEscapeEmergencyDoctrine };
