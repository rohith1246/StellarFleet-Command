/**
 * DecoyDiversionWarpEscapeEmergencyDoctrine.js - Fleet Command Tactical AI Doctrine: DecoyDiversion -> WarpEscapeEmergency.
 */

class DecoyDiversionWarpEscapeEmergencyDoctrine {
  constructor(fleetCommander, sensorRange = 50000) {
    this.doctrineName = 'DecoyDiversion';
    this.tacticName = 'WarpEscapeEmergency';
    this.commander = fleetCommander;
    this.sensorRange = sensorRange;
    this.aggressionFactor = 0.82;
  }
  ...
  evaluateBattlefield(hostileFleets, friendlyFleet) {
    ...
    return { action: 'CLOSE_DISTANCE_FORMATION', formation: 'DecoyDiversion', target: targetFleet };
  }
  ...
  execute(friendlyFleet, hostileFleets) {
    ...
    if (decision.action === 'ENGAGE_TACTICAL_MANOEUVRE') {
      ...
      friendlyFleet.repositionTowards(decision.target.pos, 'DecoyDiversion');
    }
    return decision;
  }
}

module.exports = { DecoyDiversionWarpEscapeEmergencyDoctrine };
