/**
 * CarrierStandoffWarpEscapeEmergencyDoctrine.js - Fleet Command Tactical AI Doctrine: CarrierStandoff -> WarpEscapeEmergency.
 */

class CarrierStandoffWarpEscapeEmergencyDoctrine {
  constructor(fleetCommander, sensorRange = 50000) {
    this.doctrineName = 'CarrierStandoff';
    this.tacticName = 'WarpEscapeEmergency';
    this.commander = fleetCommander;
    this.sensorRange = sensorRange;
    this.aggressionFactor = 0.34;
  }
  ...
  evaluateBattlefield(hostileFleets, friendlyFleet) {
    ...
    return { action: 'CLOSE_DISTANCE_FORMATION', formation: 'CarrierStandoff', target: targetFleet };
  }
  ...
  execute(friendlyFleet, hostileFleets) {
    ...
    if (decision.action === 'ENGAGE_TACTICAL_MANOEUVRE') {
      ...
      friendlyFleet.repositionTowards(decision.target.pos, 'CarrierStandoff');
    }
    return decision;
  }
}

module.exports = { CarrierStandoffWarpEscapeEmergencyDoctrine };
